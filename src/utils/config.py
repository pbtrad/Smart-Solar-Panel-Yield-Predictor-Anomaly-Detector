...
class ConfigManager:
            ...
            "models": {
                "forecast": {
                    ...
                    "retrain_frequency_hours": int(
                        os.getenv("RETRAIN_FREQUENCY", "168")
                    ),
                },
                "anomaly": {
    ...
    def _load_env_vars(self):
        for section in self.config:
            if isinstance(self.config[section], dict):
                self._load_nested_env_vars(self.config[section], section)

    def _load_nested_env_vars(self, config_dict: Dict[str, Any], prefix: str):
        for key, value in config_dict.items():
            if isinstance(value, dict):
                self._load_nested_env_vars(value, f"{prefix}_{key}")
            else:
    ...
    def validate_config(self) -> bool:
        ...
        if self.config["inverter"]["enabled"]:
            if (
                not self.config["inverter"]["api_url"]
                or not self.config["inverter"]["api_key"]
            ):
        ...
        if self.config["weather"]["enabled"]:
            if (
                not self.config["weather"]["api_url"]
                or not self.config["weather"]["api_key"]
            ):
