    ...
    def fetch_data(self, start_date: datetime, end_date: datetime) -> pd.DataFrame:
        ...
        for inverter_id in self.inverter_ids:
            try:
                ...
                logger.error(
                    f"Failed to fetch data for inverter {inverter_id}: {str(e)}"
                )
                continue
    ...
    def fetch_data(self, start_date: datetime, end_date: datetime) -> pd.DataFrame:
        try:
            ...
            response = self.session.get(
                f"{self.api_url}/weather/history", params=params
            )
            self.validate_response(response)
    ...
    def ingest_all_data(
        ...
        for service_name, service in self.services.items():
            try:
                ...
                logger.info(
                    f"Completed ingestion for {service_name}: {len(data)} records"
                )
