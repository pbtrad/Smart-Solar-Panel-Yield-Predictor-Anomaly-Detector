...
class TimeSeriesStorage:
    ...
    def query_power_data(
        ...
        try:
            ...
            if inverter_ids:
                ids_filter = " or ".join(
                    [f'r.inverter_id == "{id}"' for id in inverter_ids]
                )
                query += f" |> filter(fn: (r) => {ids_filter})"

            result = self.query_api.query_data_frame(
                query=query, data_frame_index=["_time"]
            )
...
class ObjectStorage:
    ...
    def store_raw_data(
        self, data: pd.DataFrame, source: str, timestamp: datetime
    ) -> str:
        try:
            key = (
                f"raw_data/{source}/"
                f"{timestamp.strftime('%Y/%m/%d')}/"
                f"{timestamp.isoformat()}.csv"
            )
    ...
    def store_model_artifacts(
        self, model_data: bytes, model_name: str, version: str
    ) -> str:
        try:
            key = f"models/{model_name}/{version}/model.pkl"

            self.s3_client.put_object(Bucket=self.bucket_name, Key=key, Body=model_data)
...
class DataStorageManager:
    ...
    def store_ingested_data(
        self, data_dict: Dict[str, pd.DataFrame], timestamp: datetime
    ) -> Dict[str, str]:
        storage_keys = {}
    ...
    def get_combined_data(
        ...
        try:
            power_data = self.ts_storage.query_power_data(
                start_time, end_time, inverter_ids
            )
            weather_data = self.ts_storage.query_weather_data(start_time, end_time)

            return {"power": power_data, "weather": weather_data}
