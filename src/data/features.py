...
class ProphetFeatureEngineer:
    ...
    def merge_weather_inverter(self, inverter_df: pd.DataFrame, weather_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def add_maintenance_features(self, df: pd.DataFrame, maintenance_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, maintenance_row in maintenance_df.iterrows():
            ...
            if maintenance_type == 'cleaning':
                mask = (df['timestamp'] >= maintenance_date) & (df['timestamp'] <= maintenance_date + timedelta(days=7))
    ...
    def prepare_prophet_data(self, df: pd.DataFrame, target_column: str = 'power_output') -> pd.DataFrame:
        if df.empty:
            return pd.DataFrame()
        ...
        regressor_columns = [
            'temp', 'humidity', 'wind_speed', 'clouds', 'solar_radiation',
            'efficiency', 'temp_efficiency', 'cloud_impact', 'weather_efficiency',
            'maintenance_flag', 'cleaning_effect'
        ]
    ...
    def process_for_prophet(self, inverter_df: pd.DataFrame, weather_df: pd.DataFrame, 
                           maintenance_df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        try:
            ...
            raise FeatureEngineeringError(f"Failed to process data for Prophet: {str(e)}")
